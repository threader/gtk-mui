 ...
 *
 * $Id: gtk_list.c,v 1.10 2011/07/04 12:16:20 o1i Exp $
 */
...
GtkType gtk_list_get_type (void) {
  ...
  if (!list_type) {
    ...
    /* as we don't have gtk_type_unique (for now), do it
     * manually:
     */
    tinfo.class_size = list_info.class_size;
    tinfo.base_init = list_info.base_class_init_func;
    tinfo.base_finalize = NULL;
    tinfo.class_init = (GClassInitFunc) list_info.class_init_func;
    tinfo.class_finalize = NULL;
    tinfo.class_data = NULL;
    tinfo.instance_size = list_info.object_size;
    tinfo.n_preallocs = 0;
    tinfo.instance_init = list_info.object_init_func;

    list_type=g_type_register_static (GTK_TYPE_CONTAINER, "GtkList", &tinfo, 0);
  }

  return list_type;
}

static void gtk_list_class_init (GtkListClass *class) {
...
#endif
  //class->selection_changed = gtk_list_selection_changed; /* added for GTK-MUI */
...
static void gtk_list_init (GtkList *list) {
  ...
  GtkSetObj(GTK_WIDGET(list), NewObject(CL_List->mcc_Class, NULL,MA_Widget,(ULONG) list, TAG_DONE));
  GTK_MUI(list)->mainclass=CL_AREA; /* ? */
  DebOut("  gtk_list_init done\n");
}
...
#endif
  return GTK_WIDGET(g_object_new (GTK_TYPE_LIST, NULL));
}
...
static gboolean list_has_grab (GtkList *list) {
...
static gint gtk_list_button_press (GtkWidget *widget, GdkEventButton *event) {
...
void gtk_list_insert_items (GtkList *list, GList   *items, gint	position) {
  ...
  DebOut("gtk_list_insert_items(%lx,%lx,%d)\n",list,items,position);
  ...
  nchildren = xget(GtkObj(list),MUIA_List_Entries);
  ...
  /* generate list */
  newchildren=g_list_length (items);
  DebOut("  newchildren: %d\n",newchildren);

  for(i=newchildren-1;i>=0;i--) {

    iw=(GtkWidget *) g_list_nth_data(items,i);
    DebOut("  iw=%lx\n",iw);
    if(!GTK_IS_LIST_ITEM(iw)) {
      ErrOut("gtk_list_insert_items with non LIST_ITEM");
    }
    else {
      /* get text of son label */
      ...
      /* insert */
      if(position == 0) {
        DoMethod(GtkObj(list),MUIM_List_Insert,(ULONG) &str,1,MUIV_List_Insert_Top);
        list->children=g_list_insert(list->children,iw,0);
      }
      else if(position > nchildren) {
        DoMethod(GtkObj(list),MUIM_List_Insert,(ULONG) &str,1,MUIV_List_Insert_Bottom);
        list->children=g_list_insert(list->children,iw,-1);
      }
      else {
        set(GtkObj(list),MUIA_List_Active,position);
        DoMethod(GtkObj(list),MUIM_List_Insert,(ULONG) &str,1,MUIV_List_Insert_Active);
        set(GtkObj(list),MUIA_List_Active,MUIV_List_Active_Off);
        list->children=g_list_insert(list->children,iw,position);
      }
    }
  }

  /* ok, but we miss quite a lot of the original gtk code, added here: */
...
void gtk_list_append_items (GtkList *list, GList   *items) {
  ...
  DebOut("gtk_list_append_items(%lx,%lx)\n",list,items);
...
/* gtk_list_child_position is deprecated and 
...
gint gtk_list_child_position (GtkList *list, GtkWidget *child) {
  ...
  DebOut("gtk_list_child_position(%lx,%lx)\n",list,child);
  ...
  while (children)
    {
      DebOut("gtk_list_child_position #%d data: %lx >%s<\n",pos,children->data,children->data);
      if (child == GTK_WIDGET (children->data)) {
	DebOut("  gtk_list_child_position found: %lx\n",child);
	f= pos;
      }
 ...
 * The signal GtkList::select-child will be emitted.
 */
void gtk_list_select_item (GtkList *list, gint     item) {
  ...
  DebOut("gtk_list_select_item(%lx,%d)\n",list,item);

  tmp_list = g_list_nth (list->children, item);
  DebOut("gtk_list_select_item: tmp_list: %lx\n",tmp_list);
  if (tmp_list) {
    gtk_list_select_child (list, GTK_WIDGET (tmp_list->data));
  }
  else {
    /* this is OK according to the GTK specs, but might be a problem
     * caused by a gtk-mui bug..? */
    WarnOut("gtk_list_select_item: could not find item nr %d\n",item);
  }
}
...
gtk_list_unselect_all (GtkList *list)
{
  ...
  DebOut("gtk_list_unselect_all(%lx)\n");
  ...
  while (work)
    {
      item = work->data;
      work = work->next;
      DebOut("call gtk_list_unselect_child(%lx,%lx)\n",list,item);
 ...
 */
static void gtk_real_list_select_child (GtkList   *list, GtkWidget *child) {
  int nr;

  g_return_if_fail (GTK_IS_LIST (list));
  g_return_if_fail (GTK_IS_LIST_ITEM (child));

  DebOut("gtk_real_list_select_child(%lx,%lx)\n",list,child);

  switch (child->state) {

    case GTK_STATE_INSENSITIVE:
      DebOut("  child is GTK_STATE_INSENSITIVE\n");
      break;
#if 0
    case GTK_STATE_SELECTED:
      DebOut("  child is already GTK_STATE_SELECTED\n");
      break;
#endif
    default:
      child->state=GTK_STATE_SELECTED; /* this is not done here in the
                                        * orginal Gtk sources, but where 
					* else?
					*/
      gtk_list_item_select (GTK_LIST_ITEM (child));
      nr=gtk_list_child_position(list,child);
      if(nr == -1) {
				WarnOut("gtk_real_list_select_child could not find child to select!\n");
      }
      else {
				DebOut(" activate nr %d\n",nr);
				/* BUG: this is quite useless here. Better would bes sth like:
				 * nnset(GtkObj(combo),MUIA_Cycle_Active,(ULONG) nr);
				 * for a combo box for example..
				 */
				set(GtkObj(list),MUIA_List_Active,nr);
      }
      break;
  }

#if 0 /* old*/
  if(child->state == GTK_STATE_INSENSITIVE) {
    return;
  }
			     ...
			     GtkList     *list)
{
  ...
  DebOut("gtk_list_signal_item_select(%lx,%lx)\n",list_item,list);

  if (GTK_WIDGET (list_item)->state != GTK_STATE_SELECTED) {
    WarnOut("gtk_list_signal_item_select on a non selected item!?\n");
    return;
  }

  DebOut("list->selection_mode: %d\n",list->selection_mode);
  ...
  switch (list->selection_mode) {
    ...
    case GTK_SELECTION_BROWSE:
      ...
      if(!list->selection) {
				DebOut("Nothing selected so far\n");
      }
      else {
				DebOut("Clear old selection\n");
      }
      while (selection) {
				tmp_list = selection;
