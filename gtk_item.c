 ...
 *
 * $Id: gtk_item.c,v 1.3 2007/06/19 13:33:16 o1i Exp $
 */
...
static void gtk_item_class_init (GtkItemClass *class) {

  GObjectClass *object_class;
  GtkWidgetClass *widget_class;

  DebOut("gtk_item_class_init(%lx)\n",class);
...
static void gtk_item_init (GtkItem *item) {
  ...
  DebOut("gtk_item_init(%lx)\n",item);

  GTK_WIDGET_UNSET_FLAGS (item, GTK_NO_WINDOW);

  GTK_MUI(item)->mainclass=CL_OTHER; /* ? */
}
...
#if 0
static gint gtk_item_enter (GtkWidget *widget, GdkEventCrossing *event) {

  g_return_val_if_fail (GTK_IS_ITEM (widget), FALSE);
  g_return_val_if_fail (event != NULL, FALSE);

  DebOut("gtk_item_enter(%lx,%lx)\n",widget,event);
