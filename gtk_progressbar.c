 ...
 *
 * $Id: gtk_progressbar.c,v 1.11 2009/05/17 18:37:48 stefankl Exp $
 *
 *****************************************************************************/
...
GType gtk_progress_bar_get_type (void) {
  ...
  DebOut("gtk_progress_bar_get_type()\n");
  ...
  if (!progress_bar_type) {
    ...
    progress_bar_type = g_type_register_static (GTK_TYPE_PROGRESS, "GtkProgressBar",
				&progress_bar_info, 0);
  }
...
static void gtk_progress_bar_class_init (GtkProgressBarClass *class) {

  GObjectClass *gobject_class;
  GtkWidgetClass *widget_class;
  GtkProgressClass *progress_class;

  DebOut("gtk_progress_bar_class_init(%lx)\n",class);
...
static void gtk_progress_bar_init (GtkProgressBar *pbar) {
  ...
  DebOut("gtk_progress_bar_init(%lx)\n",pbar);
  ...
  GtkSetObj(GTK_WIDGET(pbar), NewObject(CL_ProgressBar->mcc_Class, NULL,MA_Widget,(ULONG) pbar,TAG_DONE));
...
static void gtk_progress_bar_real_update (GtkProgress *progress) {
  ...
  DebOut("gtk_progress_bar_real_update(%lx)\n",progress);


  pbar = GTK_PROGRESS_BAR (progress);
  widget = GTK_WIDGET (progress);
 
  if (pbar->bar_style == GTK_PROGRESS_CONTINUOUS || GTK_PROGRESS (pbar)->activity_mode) {
    DebOut("gtk_progress_bar_real_update: 1\n");
    if (GTK_PROGRESS (pbar)->activity_mode) {
      guint size;
      DebOut("gtk_progress_bar_real_update: 1a\n");
      
          
      /* advance the block */

      set(GtkObj(pbar),MA_Pulse,TRUE);
#if 0
    ...
    gtk_widget_queue_draw (GTK_WIDGET (progress));
      ...
      set(GtkObj(pbar),MA_Pulse,FALSE);
    }
    ...
    DebOut("gtk_progress_bar_real_update: 2\n");
    set(GtkObj(pbar),MA_Pulse,FALSE);
#warning TODO
#if 0
    in_block = -1 + (gint)(gtk_progress_get_current_percentage (progress) * (gdouble)pbar->blocks);
...
static void gtk_progress_bar_act_mode_enter (GtkProgress *progress) {
  ...
  DebOut("gtk_progress_bar_act_mode_enter(%lx)\n",progress);

  set(GtkObj(progress),MA_Pulse,(ULONG) TRUE);
				...
				GtkProgressBarStyle bar_style)
{
  ...
  WarnOut("gtk_progress_bar_set_bar_style(%lx,%lx) ignored\n",(LONG) pbar,(LONG) bar_style);
