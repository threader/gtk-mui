 ...
 *
 * $Id: gtk_fixed.c,v 1.10 2013/07/18 12:46:08 mmartinka Exp $
 *
 *****************************************************************************/
...
static void gtk_fixed_class_init (GtkFixedClass *class) {

  GtkWidgetClass *widget_class;
  GtkContainerClass *container_class;

  DebOut("gtk_fixed_class_init(%lx)\n",class);
...
static void gtk_fixed_init (GtkFixed *fixed) {
  ...
  DebOut("gtk_fixed_init(%lx)\n",fixed);

  GtkSetObj(GTK_WIDGET(fixed),NewObject(CL_Fixed->mcc_Class, NULL,MA_Widget,(ULONG) fixed,TAG_DONE));
               ...
               gint            y)
{
  ...
  DoMethod(GtkObj(fixed),OM_ADDMEMBER,(ULONG) GtkObj(widget));
  DoMethod(GtkObj(widget),MUIM_Group_ExitChange);
#endif
  mgtk_add(GTK_MUI(fixed),GTK_MUI(widget));
...
gtk_fixed_move (GtkFixed       *fixed,
  ...
  set(GTK_MUI(fixed)->MuiObject,MA_Fixed_Move,data);
