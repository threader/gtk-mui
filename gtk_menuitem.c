 ...
 *
 * $Id: gtk_menuitem.c,v 1.3 2009/05/17 18:37:48 stefankl Exp $
 */
...
gtk_menu_item_init (GtkMenuItem *menu_item)
{
  ...
  /*****************************************
   * A menuitem is also just a Group, which
   * gets (for example) a text label through
   * gtk_menu_item_new_with_label
   *****************************************/

  GtkSetObj(GTK_WIDGET(menu_item), VGroup, 
                                        ButtonFrame,
                                        MUIA_FixHeightTxt, "X",
                                        MA_Widget,(ULONG) menu_item,
                                   End);
...
gtk_menu_item_new_with_label (const gchar *label)
{
  ...
  DebOut("gtk_accel_label_get_accel_widget: %lx\n",gtk_accel_label_get_accel_widget(GTK_ACCEL_LABEL(accel_label)));

#if 0
  GTK_MUI(menu_item)->MuiMenu =(APTR) NewObject(CL_Menu->mcc_Class, NULL,
                                                  MA_Widget,(ULONG) accel_label,
                                                  MUIA_Menuitem_Title,(ULONG) gtk_label_get_text(GTK_LABEL(accel_label)),
                                                  TAG_DONE);
#endif
...
#endif
  accel_label = g_object_new (GTK_TYPE_LABEL, NULL);
...
/******************************************
 * gtk_menu_item_set_submenu attaches a
 * submenu to an menuitem:
 * 
 * +----------------------------+
 * | menuitem |  ...            |
 * +----------+-----------------+
...
GtkWidget* mgtk_get_accel(GtkWidget *w) {
  GList *i=NULL;
  struct List *list;
  struct Node *state;
  Object *o;
  ...
  DebOut("mgtk_get_accel(%lx)\n",w);

  if(GTK_IS_ACCEL_LABEL(w)) {
    return w;
  }

  list=(struct List *) xget(GtkObj(w),MUIA_Group_ChildList);
  state = list->lh_Head;
  while ( (o = NextObject( &state )) ) {
    DebOut(" child: %lx\n",o);
    i=g_list_append(i,(GtkWidget *) xget(o,MA_Widget));
  }

  if(!i) {
    ErrOut("GTK_CONTAINER(menu_item) has no children !?\n");
    return NULL;
  }
  if(g_list_length(i)>1) {
    ErrOut("GTK_CONTAINER(menu_item) has too many children (%d)!?\n",g_list_length(i));
  }

  accel=(GtkWidget *) i->data;
  g_list_free(i);
...
void gtk_menu_item_set_submenu (GtkMenuItem *menu_item, GtkWidget   *submenu) {
  ...
  DebOut("gtk_menu_item_set_submenu(%lx,%lx)\n",menu_item,submenu);
...
#if 0
    gtk_menu_attach_to_widget (GTK_MENU (submenu), GTK_WIDGET (menu_item), gtk_menu_item_detacher);
 ...
 /*********************************************
  * For our Zune Group of the menu_item
  * we just have to activate ContextMenus.
  * They are then built, as soon as the
  * user hits the RMB by MUIM_ContextMenuBuild
  * of the CL_Group class with the help of 
  * the Gtk structures.
  *********************************************/
  ...
  */
  accel=mgtk_get_accel(GTK_WIDGET(menu_item));
  if(!accel) {
    ErrOut("gtk_menu_item_set_submenu: unable to find accel widget!\n");
    return;
  }
  set(GtkObj(accel),MUIA_ContextMenu, TRUE);   
  set(GtkObj(accel),MA_Menu_Attached, TRUE);
...
gtk_menu_item_accel_width_foreach (GtkWidget *widget,
				   gpointer data)
{
  WarnOut("gtk_menu_item_accel_width_foreach not implemented\n");
  guint *width = data;
...
{
  WarnOut("get_minimum_width not implemented\n");
  return 10;
#if 0
...
gtk_menu_item_size_request (GtkWidget      *widget,
			    GtkRequisition *requisition)
{
  WarnOut("gtk_menu_item_size_request(%lx,%lx) not yet implemeted\n",(LONG) widget,(LONG) requisition);
#if 0
  GtkMenuItem *menu_item;
  GtkBin *bin;
  guint accel_width;
...
gtk_menu_item_size_allocate (GtkWidget     *widget,
			     GtkAllocation *allocation)
{
  WarnOut("gtk_menu_item_size_allocate not implemented\n");
#if 0
  GtkMenuItem *menu_item;
  GtkBin *bin;
...
gtk_menu_item_paint (GtkWidget    *widget,
		     GdkRectangle *area)
{
  WarnOut("gtk_menu_item_paint not implemented\n");
#if 0
  GtkMenuItem *menu_item;
...
{
  WarnOut("gtk_real_menu_item_select not yet implemented\n");
#if 0
  GtkMenuItem *menu_item;
...
{
  WarnOut("gtk_menu_item_select_timeout not implemented\n");
  GtkMenuItem *menu_item;
  GtkWidget *parent;
...
gtk_menu_item_position_menu (GtkMenu  *menu,
			     ...
			     gpointer  user_data)
{
  WarnOut("gtk_menu_item_position_menu not implemented\n");
#if 0
  GtkMenuItem *menu_item;
  GtkWidget *widget;
...
gtk_menu_item_show_all (GtkWidget *widget)
{
  ...
  WarnOut("gtk_menu_item_show_all not done yet!\n");
...
gtk_menu_item_hide_all (GtkWidget *widget)
{
  ...
  WarnOut("gtk_menu_item_hide_all not done yet!\n");
...
gtk_menu_item_can_activate_accel (GtkWidget *widget,
				  guint      signal_id)
{
  WarnOut("gtk_menu_item_can_activate_accel not implemented\n");
  return FALSE;
#if 0
				  ...
				  gpointer data)
{
  ...
  WarnOut("gtk_menu_item_accel_name_foreach not doen yet\n");
...
{
  WarnOut("_gtk_menu_item_refresh_accel_path not implemented\n");
#if 0
  const gchar *path;
  GtkWidget *widget;
...
{
  WarnOut("gtk_menu_item_set_accel_path not implemented\n");
#if 0
  GtkWidget *widget;
