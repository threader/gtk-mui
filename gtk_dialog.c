 ...
 *
 * $Id: gtk_dialog.c,v 1.9 2009/05/17 18:37:48 stefankl Exp $
 */
...
static void gtk_dialog_class_init (GtkDialogClass *class) {
  ...
  DebOut("gtk_dialog_class_init(%lx)\n",class);
...
static void gtk_dialog_init (GtkDialog *dialog) {
  ...
  if(obj) {
    DebOut("  new gtk window %lx, mui %lx\n",dialog,obj);
      ...
      GtkSetObj(GTK_WIDGET(dialog),root);
      GTK_MUI(dialog)->MuiWin          = obj;
      GTK_MUI(dialog)->mainclass       = CL_WINDOW;

      DebOut("GTK_MUI(win)->MuiWin          = %lx\n",obj);
      DebOut("GTK_MUI(win)->MuiObject       = %lx\n",root);

      global_mgtk->windows=g_slist_append(global_mgtk->windows,dialog);

  }
  else {
    ErrOut("gtk_dialog.c: unable to create dialog window!\n");
  }
  ...
  dialog->action_area = gtk_hbox_new (0,4);
...
{
  /* nothing we need to do.. or ? */

  WarnOut("gtk_dialog_map() is disabled\n");
#if 0
...
static void gtk_dialog_style_set (GtkWidget *widget, GtkStyle  *prev_style) {
		    ...
		    gint       response_id)
{
  ...
  ErrOut("dialog_find_button(%lx,..) not done yet!\n",dialog);
  return NULL;
...
#endif
#warning enable gtk_window_set_destroy_with_parent again!
...
gtk_dialog_add_action_widget (GtkDialog *dialog,
  ...
  ErrOut("gtk_dialog_add_action_widget(%lx,%lx,%d) is MISSING\n",dialog,child,response_id);
#if 0
  ResponseData *ad;
  guint signal_id;
                                   ...
                                   gboolean   setting)
{
  ...
  ErrOut("gtk_dialog_set_response_sensitive not done yet!\n");
#if 0
  GList *children;
  GList *tmp_list;
                                 ...
                                 gint       response_id)
{
  ...
  ErrOut("gtk_dialog_set_default_response(..) not done yet!\n");
  return;
...
{
  ErrOut("gtk_dialog_run(%lx) needs to be done!\n");
#if 0
...
{
  ErrOut("gtk_alternative_dialog_button_order not done yet\n");
  return FALSE;
#if 0
...
gtk_dialog_set_alternative_button_order (GtkDialog *dialog,
					 ...
					 ...)
{
  WarnOut("gtk_dialog_set_alternative_button_order not done yet\n");
  return;
#if 0
  GdkScreen *screen;
  va_list args;
...
gtk_dialog_set_alternative_button_order_from_array (GtkDialog *dialog,
                                                    ...
                                                    gint      *new_order)
{
  WarnOut("gtk_dialog_set_alternative_button_order_from_array not done yet\n");
#if 0
  GdkScreen *screen;
  GtkWidget *child;
  gint position;
