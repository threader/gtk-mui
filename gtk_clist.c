 ...
 *
 * $Id: gtk_clist.c,v 1.5 2009/05/17 18:37:48 stefankl Exp $
 */
...
static void gtk_clist_class_init (GtkCListClass *klass) {
  ...
  DebOut("gtk_clist_class_init(%lx)\n",klass);
...
GtkWidget* gtk_clist_new_with_titles (gint columns, gchar *titles[]) {
  ...
  DebOut("gtk_clist_new_with_titles(%d,%lx)\n",columns,titles);

  clist = g_object_new (GTK_TYPE_CLIST,
			"n_columns", columns,
			NULL);

  GtkSetObj(GTK_WIDGET(clist),NewObject(CL_CList->mcc_Class, NULL,
                                          MA_Widget,          (ULONG) clist, 
                                          MA_CList_Columns,   (ULONG) clist->columns,
                                        TAG_DONE));
...
void gtk_clist_column_titles_show (GtkCList *clist) {
  ...
  DebOut("gtk_clist_column_titles_show(%lx)\n",clist);
  ...
  if (!GTK_CLIST_SHOW_TITLES(clist)) {
	...
	gdk_window_show (clist->title_window);
      gtk_widget_queue_resize (GTK_WIDGET (clist));
#endif
      set(GtkObj(clist),MUIA_List_Title,TRUE);
    }
}
...
void gtk_clist_column_titles_hide (GtkCList *clist) {
  ...
  DebOut("gtk_clist_column_titles_hide(%lx)\n",clist);
  ...
  if (GTK_CLIST_SHOW_TITLES(clist))
	...
	gdk_window_hide (clist->title_window);
      gtk_widget_queue_resize (GTK_WIDGET (clist));
#endif
      set(GtkObj(clist),MUIA_List_Title,FALSE);
    }
}
...
gtk_clist_column_title_passive (GtkCList *clist,
				gint      column)
{
  WarnOut("gtk_clist_column_title_passive disabled\n");
#if 0
  GtkButton *button;
			    ...
			    const gchar *title)
{
  ...
  DebOut("gtk_clist_set_column_title(%lx,%d,%s)\n",clist,column,title);
...
#warning gtk_alignment_new is TODO!
  DebOut("WARNING: GTK_JUSTIFY is TODO!..\n");
#if 0
...
//  ErrOut("we would crash here, because of alignment=NULL!\n");
...
{
WarnOut("gtk_clist_set_column_justification not done yet!\n");
#if 0
  GtkWidget *alignment;
...
gtk_clist_set_pixtext (GtkCList    *clist,
		       ...
		       GdkBitmap   *mask)
{
WarnOut("gtk_clist_set_pixtext not DONE\n");
#if 0
  GtkCListRow *clist_row;
		   ...
		   GdkBitmap   *mask)
{
  ...
  DebOut("set_cell_contents(%lx,%lx,%d,..,%s,..) IS REALLY TODO!\n",clist,clist_row,column,text);
#if 0
  if (clist->column[column].auto_resize &&
      !GTK_CLIST_AUTO_RESIZE_BLOCKED (clist))
    GTK_CLIST_GET_CLASS (clist)->cell_size_request (clist, clist_row,
  ...
  switch (type)
    ...
    case GTK_CELL_EMPTY:
      break;
    default:
      WarnOut("  gtk_clist.c: unimplemented GTK_CELL_TYPE: %d\n",clist_row->cell[column].type);
      break;
    }
		 ...
		 gchar    *text[])
{
  ...
  if(clist->rows>1) {
#endif
    /* we already have one dummy row in our MUI object, so do nothing for the first row */
#ifdef MGTK_DEBUG
    DoMethod(GtkObj(clist),MUIM_List_InsertSingle,(ULONG) "-add: empty-",MUIV_List_Insert_Bottom);
#else
    DoMethod(GtkObj(clist),MUIM_List_InsertSingle,(ULONG) " ",MUIV_List_Insert_Bottom);
#endif
#if 0
  }
#endif
		 ...
		 gint      row)
{
  ...
  clist->rows--;
  if(clist->rows>1) {
    /* we must keep the last row in MUI */
    DoMethod(GtkObj(clist), MUIM_List_Remove, (ULONG) row);
  }
...
real_clear (GtkCList *clist)
{
  ...
  DebOut("gtk_clist.c: real_clear(%lx)\n",clist);
  ...
  DoMethod(GtkObj(clist),MUIM_List_Clear);
			  ...
			  gint      row)
{
  ...
  DebOut("gtk_clist_row_is_visible(%lx,%d)\n",clist,row);
  ...
  top=xget(GtkObj(clist),MUIA_List_First);
  len=xget(GtkObj(clist),MUIA_List_Visible);
			...
			GtkRequisition *requisition)
{
  ...
  DebOut("gtk_clist_size_request(%lx,%lx)\n",widget,requisition);
  ...
  if (GTK_CLIST_SHOW_TITLES(clist))
      ...
      if (clist->column[i].button)
	{
	  ...
	  child_requisition.gtkmui_type=2;
	  ...
	  GtkCListRow  *clist_row)
{
  ...
  DebOut("gtk_clist.c: draw_row(%lx,..,%d,..)\n",clist,row);

  DoMethod(GtkObj(clist),MUIM_List_Redraw,row);
#if 0
  GtkWidget *widget;
  GdkRectangle *rect;
...
static void draw_rows (GtkCList *clist, GdkRectangle *area) {
...
#if 0
  DoMethod(GtkObj(clist), MUIM_Group_InitChange);
  DoMethod(GtkObj(clist), MUIM_Group_ExitChange);
#endif
