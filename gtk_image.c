 ...
 *
 * $Id: gtk_image.c,v 1.12 2007/06/19 13:33:16 o1i Exp $
 */
...
static void gtk_image_class_init (GtkImageClass *class) {

  GObjectClass *gobject_class;
  GtkObjectClass *object_class;
  GtkWidgetClass *widget_class;

  DebOut("gtk_image_class_init(%lx)\n",class);
...
gtk_image_destroy (GtkObject *object)
{
  ...
  DebOut("gtk_image_destroy(%lx)\n");

#if 0
  gtk_image_clear (image);
#endif
  if(GTK_MUI(object)->MuiObject) {
    if(!xget(GTK_MUI(object)->MuiObject,MUIA_Parent)) {
      DoMethod(mgtk->MuiApp,MM_GtkApplication_AddLoner,(ULONG) GTK_MUI(object)->MuiObject);
      /*
      DebOut("DisposeObject(%lx)\n",GTK_MUI(object)->MuiObject);
      DisposeObject(GTK_MUI(object)->MuiObject);
      */
    }
  }
			...
			GParamSpec   *pspec)
{
  ...
  DebOut("gtk_image_set_property(%lx,..)\n",object);
			...
			GParamSpec  *pspec)
{
  ...
  DebOut("gtk_image_get_property(%lx,..)\n",object);
...
void gtk_image_set_from_file   (GtkImage    *image, const gchar *filename) {
  ...
  DebOut("gtk_image_set_from_file(%lx,%s)\n",image,filename);
  ...
  GtkSetObj(GTK_WIDGET(image), pic);
...
static void gtk_image_reset (GtkImage *image) {
  ...
  WarnOut("gtk_image_reset(%lx) does nothing..\n",(LONG) image);
#if 0
  gtk_image_clear (image);
