 ...
 *
 * $Id: gtk_settings.c,v 1.4 2009/05/17 18:37:48 stefankl Exp $
 */
...
static void gtk_settings_init (GtkSettings *settings) {
  ...
  DebOut("gtk_settings_init(%ls)\n",settings);

  g_datalist_init (&settings->queued_settings);
  object_list = g_slist_prepend (object_list, settings);

  GTK_MUI(settings)->mainclass=CL_OTHER;
...
static void gtk_settings_class_init (GtkSettingsClass *class) {
  ...
  DebOut("gtk_settings_class_init(%lx)\n",class);
...
static void gtk_settings_get_property (GObject     *object,
  ...
  DebOut("gtk_settings_get_property(%lx,%d,..)\n",object,property_id);
...
*/
    ...
    WarnOut("_gtk_rc_property_parser_from_type: no parser for type %d\n",type);
    return NULL;
  }
