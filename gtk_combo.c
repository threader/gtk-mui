 ...
 *
 * $Id: gtk_combo.c,v 1.6 2011/07/04 12:16:20 o1i Exp $
 */
...
static void gtk_combo_selection_changed (GtkList  *list, GtkCombo *combo) {
  gint nr;

  DebOut("(%lx,%lx)\n",list,combo);

  /* better check, than die ..*/
  if(!list) {
    ErrOut("gtk_combo_selection_changed: list is NULL\n");
    return;
  }
  if(!list->selection) {
    ErrOut("gtk_combo_selection_changed: list->selection is NULL\n");
    return;
  }
  if(!list->selection->data) {
    ErrOut("gtk_combo_selection_changed: list->selection->data is NULL\n");
    return;
  }

  /* action */
  nr=gtk_list_child_position(list,(GtkWidget *) list->selection->data);

  /* checks again */
  if(nr == -1) {
    WarnOut("gtk_combo_selection_changed: could not find child to activate!\n");
    return;
  }
  DebOut("nr=%d\n",nr);
...
#if MGTK_DEBUG
  if(nr<0 || nr>10000) {
    ErrOut("gtk_combo_selection_changed: nr=%d\n",nr);
    return;
  }
#endif

  DebOut("GtkObj(%lx)=%lx\n",combo,GtkObj(combo));
  DebOut("nnset(GtkObj(%lx),MUIA_Cycle_Active,(ULONG) %d);\n",combo,nr);
  ...
  DebOut("gtk_combo_selection_changed(%lx,%lx) done\n",list,combo);
}
...
void gtk_combo_check() {
  ULONG foo;

  DebOut("--> started for %lx<--\n",checkme);
  if(!checkme) {
    DebOut("  checkme still NULL\n");
  }
  else {

    foo=xget(checkme,MUIA_Parent);
    DebOut("seems to be ok, parent is %lx\n",foo);
  }
}
#endif
...
static void gtk_combo_init (GtkCombo * combo) {
  ...
  DebOut("  NewObject..\n");
  /* create a MUI Object */
  GtkSetObj(GTK_WIDGET(combo),NewObject(CL_Combo->mcc_Class, NULL,MA_Widget,(ULONG) combo, TAG_DONE));

  DebOut("  gtk_combo_init done\n");
}
...
void gtk_combo_set_value_in_list (GtkCombo * combo, gboolean val, gboolean ok_if_empty) {
  ...
  DebOut("gtk_combo_set_value_in_list(%lx,%d,%d)\n",combo,val,ok_if_empty);

  if(!val || ok_if_empty) {
    ErrOut("gtk-mui combo only supports: no manual values and no ok_if_empty (%d,%d)\n",val,ok_if_empty);
  }
#if 0
...
void gtk_combo_set_use_arrows (GtkCombo * combo, gboolean val) {
  ...
  DebOut("gtk_combo_set_use_arrows(%lx,%d) ignored\n",combo,val);
...
void gtk_combo_set_popdown_strings (GtkCombo *combo, GList    *strings) {
  ...
  DebOut("gtk_combo_set_popdown_strings(%lx,..)\n",combo);
  ...
  /* we don't use this stuff in MUI, but someone might want
   * to peek around in it..
   */
  ...
  /* now send the list to GTK-MUI to update
   * the Object
   */

  DoMethod(GtkObj(combo),MM_Combo_Itemlist_new, (ULONG) strings);
  DebOut("  return from gtk_combo_set_popdown_strings(%lx,..)\n",combo);
}
