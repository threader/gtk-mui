 ...
 *
 * $Id: gtk_adjustment.c,v 1.5 2012/05/10 08:42:11 o1i Exp $
 */
...
GType gtk_adjustment_get_type (void) {
  ...
  DebOut("gtk_adjustment_get_type()\n");
  ...
  if (!adjustment_type) {
      ...
      adjustment_type = g_type_register_static (GTK_TYPE_OBJECT, "GtkAdjustment",
                                                &adjustment_info, 0);
    }
  return adjustment_type;
}

static void gtk_adjustment_class_init (GtkAdjustmentClass *class) {
...
static void gtk_adjustment_init (GtkAdjustment *adjustment) {
  ...
  GTK_MUI(adjustment)->mainclass=CL_OTHER;
}
...
void gtk_adjustment_value_changed (GtkAdjustment        *adjustment) {
  ...
  DebOut("gtk_adjustment_value_changed(%lx,..)\n",adjustment);
   ...
   * but we have all real Objects in the remember GSList 
   */

  g=GTK_MUI(adjustment)->remember;
  
  DebOut("g: %lx\n", g);
  while(g) {
    w=g->data;
  
    DebOut("gtk_adjustment_value_changed is still quite unfinished..\n");
#if 0
    if(w->step_increment) {
      /* slider */
    ...
    //v=(ULONG) GTK_ADJUSTMENT(w)->value;
    v=GTK_RANGE(w)->adjustment->value / GTK_RANGE(w)->adjustment->step_increment;
    DebOut("new value: %d\n", v);
    set(GtkObj(w),MUIA_Numeric_Value,v);
