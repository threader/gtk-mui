 ...
 *
 * $Id: gtk_container.c,v 1.10 2008/08/07 10:06:20 o1i Exp $
 */
...
static void gtk_container_base_class_init (GtkContainerClass *class) {
...
static void gtk_container_base_class_finalize (GtkContainerClass *class) {
...
static void gtk_container_class_init (GtkContainerClass *class) {

  GObjectClass *gobject_class = G_OBJECT_CLASS (class);
  GtkObjectClass *object_class = GTK_OBJECT_CLASS (class);
  GtkWidgetClass *widget_class = GTK_WIDGET_CLASS (class);

  DebOut("gtk_container_class_init(%lx)\n",class);
...
#warning child property: Can be used to add a new child to the container..??
  g_object_class_install_property (gobject_class,
                                   PROP_CHILD,
...
#warning _gtk_widget_child_property_notify_context might be needed here..
#if 0
					    ...
					    GParamSpec        *pspec)
{
  ...
  DebOut("gtk_container_class_install_child_property(%lx,%d,..)\n",cclass,property_id);
  ...
  DebOut("gtk_container_class_install_child_property: g_param_spec_pool_lookup is disabled\n");
#if 0
...
gtk_container_init (GtkContainer *container)
{
  ...
  GTK_MUI(container)->mainclass=CL_AREA;
}

static void
gtk_container_destroy (GtkObject *object)
{
#warning TODO: enable gtk_container_destroy again
#if 0
				...
				guint         border_width)
{
  ...
  DebOut("gtk_container_set_border_width: ignored (use MUI defaults)\n");
}
...
void gtk_container_add (GtkContainer *container, GtkWidget    *widget) {
  ...
  DebOut("gtk_container_add(%lx,%lx)\n",container,widget);
  DebOut("  container is a %s\n",g_type_name(G_OBJECT_TYPE (container)));
  ...
  /* hmm.. seems to be not needed any more*/
  if(GTK_IS_FRAME(container) && !GTK_BIN(container)->child) {
    DebOut("GTK_BIN(frame)->child is NULL, fix it by force!?\n");
    //GTK_BIN(container)->child=widget;
  }
  DebOut("  ===============\n");
  DebOut("  emit ADD signal\n");
  DebOut("  ==           ==\n");
  g_signal_emit (container, container_signals[ADD], 0, widget);
  DebOut("  ==           ==\n");
  DebOut("  ADD signal done\n");
  DebOut("  ===============\n");

  /* due to some magic, the signal ADD might already have called
   * the correct add function (box_pack_start for example),
   * so we don't have to do a mui_container add here.
   * But if our mui object has not beed added up to now,
   * we do it manually. Not nice, but should work.
   */
#if 0
  if(!GTK_IS_LIST_ITEM(widget)) {
    /* listitems are not shown by the container, otherwise, they
     * are added as a normal object to the container in MUI
     */
    muiparent=xget(GtkObj(widget),MUIA_Parent);
    DebOut("  mui parent of widget (mui obj %lx)%lx: %lx\n",widget,GtkObj(widget),muiparent);
    if(!muiparent) {
      DebOut("  manually call mgtk_container_add\n");
      mgtk_container_add(container,widget);
    }
  }
#endif
...
{
  WarnOut("gtk_container_set_resize_mode(%lx,%d) not yet implemented\n",container,resize_mode);
#if 0
  g_return_if_fail (GTK_IS_CONTAINER (container));
...
gtk_container_show_all (GtkWidget *widget)
{
  ...
  DebOut("gtk_container_show_all(%lx)\n",widget);
