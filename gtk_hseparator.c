 ...
 *
 * $Id: gtk_hseparator.c,v 1.3 2007/03/19 09:00:06 o1i Exp $
 */
...
GType gtk_hseparator_get_type (void) {
  ...
  if (!hseparator_type) {
    ...
    hseparator_type = g_type_register_static (GTK_TYPE_SEPARATOR, "GtkHSeparator", &hseparator_info, 0);
  }

  return hseparator_type;
}

static void gtk_hseparator_class_init (GtkHSeparatorClass *class) {
...
static void gtk_hseparator_init (GtkHSeparator *hseparator) {
  ...
  DebOut("gtk_hseparator_init(lx)\n",hseparator);

  GTK_WIDGET (hseparator)->requisition.width = 1;
  GTK_WIDGET (hseparator)->requisition.height = GTK_WIDGET (hseparator)->style->ythickness;

  GtkSetObj(GTK_WIDGET(hseparator),RectangleObject, MUIA_Rectangle_HBar, TRUE, MUIA_FixHeight, 8, End);
}
