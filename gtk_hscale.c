 ...
 *
 * $Id: gtk_hscale.c,v 1.5 2012/05/10 08:42:11 o1i Exp $
 */
...
static void gtk_hscale_class_init (GtkHScaleClass *class) {

  GtkWidgetClass *widget_class;
  GtkRangeClass *range_class;
  GtkScaleClass *scale_class;

  DebOut("gtk_hscale_class_init(%lx)\n",class);
...
static void gtk_hscale_init (GtkHScale *hscale) {
  ...
  GtkSetObj(GTK_WIDGET(hscale),NewObject(CL_Scale->mcc_Class, NULL,
                                       MA_Widget,(ULONG) hscale, 
  ...
  if(!GTK_MUI(hscale)->MuiObject) {
    ErrOut("unable to create GTK_MUI(hscale)->MuiObject\n");
  }
  else {
    DoMethod(GtkObj(hscale),MM_Scale_Pos,GTK_POS_TOP);
  }

  GTK_MUI(hscale)->mainclass=CL_AREA;
...
GtkWidget* gtk_hscale_new (GtkAdjustment *adjustment) {
  ...
  DebOut("gtk_hscale_new(%lx)\n",adjustment);

  w=g_object_new (GTK_TYPE_HSCALE, "adjustment", adjustment, NULL);
  ...
  if(!GTK_MUI(w)->MuiObject) {
    ...
    DoMethod(GTK_MUI(w)->MuiObject,MM_Scale_Pos,GTK_POS_TOP);
    GTK_MUI(adjustment)->remember=g_slist_append(GTK_MUI(adjustment)->remember, w);
  }

  DoMethod(GtkObj(w),MM_Scale_Redraw,(ULONG) TRUE);
