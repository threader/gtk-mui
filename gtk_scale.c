 ...
 *
 * $Id: gtk_scale.c,v 1.3 2007/06/19 13:33:18 o1i Exp $
 */
...
static void gtk_scale_class_init (GtkScaleClass *class) {
  ...
  DebOut("gtk_scale_class_init(%lx)\n",class);
...
#warning enable value_pos property again!
#if 0
  g_object_class_install_property (gobject_class,
                                   PROP_VALUE_POS,
...
static void gtk_scale_init (GtkScale *scale) {
  ...
  GTK_MUI(scale)->mainclass=CL_AREA;
}
...
void gtk_scale_set_digits (GtkScale *scale, gint      digits) {
  ...
  DebOut("gtk_scale_set_digits(%lx,%d)\n",scale,digits);
  ...
  if (scale->digits != digits) {
    ...
    DoMethod(GtkObj(scale),MM_Scale_Redraw,(ULONG) TRUE);
  }
}
...
#warning enable value-pos again
#if 0
