 ...
 *
 * $Id: notebook.c,v 1.3 2011/07/04 12:16:21 o1i Exp $
 */
...
//    gtk_notebook_set_tab_pos (GTK_NOTEBOOK (notebook), GTK_POS_TOP);
	...
	button= gtk_button_new_with_label(bufferf);
	gtk_widget_show (button);
	gtk_container_add (GTK_CONTAINER (vbox), button);

	items=g_list_append (items, bufferf);
	items=g_list_append (items, "nothing to see here");
	items=g_list_append (items, "and here ..");
	items=g_list_append (items, "and here!");
