 ...
 *
 * $Id: gtk_widget.c,v 1.13 2013/03/25 10:22:48 o1i Exp $
 */
...
gtk_widget_class_init (GtkWidgetClass *klass)
{
  ...
  klass->selection_clear_event = NULL;
  klass->selection_request_event = NULL;
  klass->selection_notify_event = NULL;
#endif
...
gtk_widget_init (GtkWidget *widget)
{
  ...
  GtkSetObj(widget, NewObject(CL_Custom->mcc_Class, NULL,MA_Widget,(ULONG) widget,TAG_DONE));

  GTK_MUI(widget)->mainclass=CL_OTHER;
}
...
gtk_widget_unparent (GtkWidget *widget)
{
   ...
   */
#warning TODO: GTK_WIDGET_ANCHORED
#if 0
  ...
  if (GTK_WIDGET_REALIZED (widget)) 
    {
      if (GTK_WIDGET_IN_REPARENT (widget)) {
        WarnOut("gtk_widget_unmap to be called, but we don't have that (missing)\n");
...
void gtk_widget_destroy (GtkWidget *widget) {
  ...
  if(GTK_IS_FILE_SELECTION(widget)) {
    DebOut("WARNING: gtk_widget_destroy(%lx) for FILE_SELECTION ignored!\n", widget);
    return;
  }

  if(GTK_IS_WINDOW(widget) && GTK_MUI(widget)->mainclass==CL_WINDOW) {
    destroywin=1;
    win=GTK_MUI(widget)->MuiWin;
  }

#warning TODO: gtk_object_destroy ..
//  gtk_object_destroy ((GtkObject*) widget);

  if(destroywin) {
    if(win) {
      DebOut("  remember to destroy mui window %lx\n",win);
      set(win,MUIA_Window_Open,FALSE);
      /* if we do Dispose it now, it is likely, that the window contained a "Close" button,
       * that triggered the gtk_widget_destroy. We are killing ourselves, if
       * we do the following:

      DoMethod(global_mgtk->MuiApp, OM_REMMEMBER, (ULONG) win);
      MUI_DisposeObject(win);

       * So we leave this to the main loop:
       mgtk->dead_windows=g_list_append(mgtk->dead_windows,(gpointer) win);
       * or do it throudg AddLoner?
       */
       mgtk->windows=g_slist_remove(mgtk->windows,widget); /* TODO: THIS IS NOT WORKING.. */
       DebOut("mgtk->windows len: %d\n",g_slist_length(mgtk->windows));
			 /* o1i: disabled, as it crashes on exit.. not sure, if this magic is necessary at all.. 
        * DoMethod(mgtk->MuiApp,MM_GtkApplication_AddLoner,(ULONG) win);
				*/
       DebOut("mgtk->windows len: %d\n",g_slist_length(mgtk->windows));
       mgtk_check_windows();
    }
    else {
      ErrOut("gtk_widget_destroy on gtk window %lx, which has no MuiWin attached!?\n",widget);
    }
  }
}
...
#warning TODO: gtk_main_iteration
#if 0
      while (!flag)
        gtk_main_iteration ();
#endif
...
void gtk_widget_realize (GtkWidget *widget) {
  ...
  if (!GTK_WIDGET_REALIZED (widget)) {
    ...
    if (widget->parent == NULL && !GTK_WIDGET_TOPLEVEL (widget))
  ...
  DebOut("================= gtk_widget_realize (%lx) left ==================\n", widget);
}
...
gtk_widget_queue_draw (GtkWidget *widget)
{
  ...
  mgtk_widget_queue_draw(widget);
#if NEED_CALLBACKS_HMMM
		 ...
		 GdkRectangle *area)
{
  ...
  DebOut("gtk_widget_draw(%lx,%lx): is just a dummy\n",widget,area);
			 ...
			 GtkRequisition *requisition)
{
  ...
  DebOut("gtk_widget_size_request(%lx,%lx)\n",widget,requisition);
  ...
  DebOut("  widget is a: %s\n",g_type_name(G_OBJECT_TYPE (widget)));

  switch(requisition->gtkmui_type) {
    case 1:
      requisition->width  = _minwidth  (GtkObj(widget));
      requisition->height = _minheight (GtkObj(widget));
      DebOut("  requisition->gtk_muitype:  minwidth\n");
      break;
    case 2:
      requisition->width  = _defwidth  (GtkObj(widget));
      requisition->height = _defheight (GtkObj(widget));
      DebOut("  requisition->gtk_muitype:  defwidth\n");
      break;
    case 3:
      requisition->width  = _maxwidth  (GtkObj(widget));
      requisition->height = _maxheight (GtkObj(widget));
      DebOut("  requisition->gtk_muitype:  maxwidth\n");
      break;
    default:
      requisition->width  = 0;
      requisition->height = 0;
      WarnOut("  requisition->gtk_muitype:  default?\n");
  }

  DebOut("  requisition->width:  %d\n",requisition->width);
  DebOut("  requisition->height: %d\n",requisition->height);
				  ...
				  GtkRequisition *requisition)
{
  ...
  /* for gtk-mui we currently only use this for table
   * calulations.
   ...
   * };
   *
   * 0: not set, do nothing.
   * 1: minwidth
   * 2: defwidth
   * 3: maxwidth
   */

  DebOut("gtk_widget_get_child_requisition(%lx,%lx,%d)\n",widget,requisition,requisition->gtkmui_type);
  ...
  switch(requisition->gtkmui_type) {
    ...
    default:
      requisition->width  = 0;
      requisition->height = 0;
      DebOut("  requisition->gtk_muitype:  default?\n");
  }
...
{
  DebOut("gtk_widget_queue_shallow_draw is just a dummy\n");
#if 0
  GdkRegion *region;
 ...
 *
 * This function is in a real GTK environment only used by #
 * GtkContainer subclasses, to assign a size
 * and position to their child widgets. 
 *
 * GTK-MUI uses this only for tables ATM!
 * So it might not work for anything else!
 * 
 **/
void
gtk_widget_size_allocate (GtkWidget	*widget,
  ...
  DebOut("gtk_widget_size_allocate(%lx,%lx)\n",widget,allocation);

  DebOut("  widget type is:   %s\n",g_type_name(G_OBJECT_TYPE (widget)));
  parent = gtk_widget_get_parent (widget);
  DebOut("  parent widget is: %s (%lx)\n",g_type_name(G_OBJECT_TYPE (parent)),parent);

  if(parent && !GTK_IS_TABLE(parent) && !GTK_IS_HBOX(parent) && !GTK_IS_VBOX(parent)) {
    ErrOut("gtk_widget_size_allocate called for %lx,\nwhich parent %lx is not a table/hbox/vbox!\n",widget,parent);
    return;
  }

  DebOut("  x-offset: %d y-offset: %d\n",allocation->x, allocation->y);
  DebOut("  width:    %d height:   %d\n",allocation->width, allocation->height);

  if(allocation->width && allocation->height) {
    if (!MUI_Layout(GtkObj(widget),
	  allocation->x,
	  allocation->y,
	  allocation->width,
	  allocation->height,
	  0)) {
      WarnOut("MUI_Layout returned FALSE for widget %x (obj %x)\n",(ULONG) widget,(ULONG) GtkObj(widget));
    }
  }
  else {
    WarnOut("width/height of widget %x is 0!\n", (ULONG) widget);
  }
}

#if 0
/* original GTK gtk_widget_size_allocate: */
/**
 * gtk_widget_size_allocate:
 * @widget: a #GtkWidget
 * @allocation: position and size to be allocated to @widget
		      ...
		      GtkStateType         state)
{
  ...
  mgtk_widget_set_state(widget,state);
			  ...
			  gboolean   sensitive)
{
  ...
  mgtk_widget_set_sensitive(widget,sensitive);
		       ...
		       GtkWidget *parent)
{
  ...
  DebOut("gtk_widget_set_parent(%lx,%lx)\n",widget,parent);
...
{
  DebOut("gtk_widget_reset_rc_style(%lx) is just a dummy\n",widget);
...
reset_rc_styles_recurse (GtkWidget *widget, gpointer data)
{
  ...
  DebOut("reset_rc_styles_recurse(%lx,%lx)\n",widget,data);
...
gtk_widget_reset_rc_styles (GtkWidget *widget)
{
  ...
  DebOut("gtk_widget_reset_rc_styles(%lx) is just a dummy\n",widget);
...
{
  DebOut("gtk_widget_get_default_style()\n");
...
gtk_widget_has_screen (GtkWidget *widget)
{
  ...
  WarnOut("gtk_widget_has_screen(%x) always returns FALSE\n",(ULONG) widget);
			       ...
			       gint       height)
{
  ...
  DebOut("gtk_widget_set_usize_internal(%lx,%d,%d) is not perfectly working (2)..\n",widget,width,height);

  g_return_if_fail (GTK_IS_WIDGET (widget));

  /* hmm, and this means for MUI..? */

  if(!GtkObj(widget)) {
    DebOut("  widget has no MUI object\n");
    return;
  }

  /* widget or height == -1 means unlimited in Gtk and MUI (AskMin_Unlimited) */

  if(width > 0) {
    set(GtkObj(widget),MUIA_FixWidth,width);
  }

  if(height > 0) {
    set(GtkObj(widget),MUIA_FixHeight,height);
  }
...
#endif
  DebOut("gtk_widget_set_usize_internal left\n");
}
 ...
 **/
void mgtk_widget_set_size_request(GtkWidget           *widget, gint                 width, gint                 height);
void
                             ...
                             gint       height)
{
  ...
  gtk_widget_set_usize_internal (widget, width, height);
#endif
  mgtk_widget_set_size_request(widget,width,height);
}
...
gtk_widget_get_colormap (GtkWidget *widget)
{
  ...
  DebOut("return widget->style->colormap: %lx\n", widget->style->colormap);
...
*/
/* 
 * oli: this is more than ugly! the visual should
 *      already be there, not created here!
 */
...
#warning =============> FIX THIS VISUAL THING <======================
  if(!widget->visual) {
    DebOut("gtk_widget_get_visual(%lx)\: unable to get visual..\n",widget);
    widget->visual=g_new(GdkVisual,1);
    /* we might need that! */
    widget->visual->mgtk_widget=(int *) widget;
    DebOut("widget %lx now has a visual: %lx\n", widget, widget->visual);
  }
...
gtk_widget_propagate_state (GtkWidget           *widget,
			    GtkStateData        *data)
{
  WarnOut("gtk_widget_propagate_state(%lx,%lx) is just a dummy\n",widget,data);
					 ...
					 GParamSpec     *pspec)
{
  ...
  DebOut("TODO: gtk_widget_class_install_style_property(%lx,%lx) missing!!\n",klass,pspec);
  return;
...
#warning TODO: gtk_widget_style_get_property
  g_warning ("%s: gtk_widget_style_get_property not working yet, so widget class `%s' has no property named `%s'",
	       G_STRLOC,
	       G_OBJECT_TYPE_NAME (widget),
	       property_name);
  return;
		      ...
		      ...)
{
  ...
  WarnOut("gtk_widget_style_get(%x,..) is not yet implemented\n",(LONG) widget);
